...
SPARK_SCRIPT_PATH="/home/${SSH_USER}/pog/cluster/Spark_sql_similarity_cluster.py"

# Directory dove i log e i tempi verranno temporaneamente salvati sul nodo master EMR
LOCAL_LOG_DIR_ON_EMR="/home/${SSH_USER}/spark_logs_model_similarity/"
LOCAL_LOG_FILE_ON_EMR="${LOCAL_LOG_DIR_ON_EMR}model_similarity_results.txt"
LOCAL_TIMES_FILE_ON_EMR="${LOCAL_LOG_DIR_ON_EMR}model_similarity_exec_times.txt"
...
LOCAL_GRAPH_FILE_PATH="${LOCAL_RESULTS_DIR}model_similarity_exec_times.png"
LOCAL_TIMES_FILE_PATH="${LOCAL_RESULTS_DIR}model_similarity_exec_times.txt"

# Assicurati che la directory locale per i risultati esista
mkdir -p "${LOCAL_RESULTS_DIR}"

echo "Starting Spark SQL Model Similarity job on EMR cluster..."
...
plt.title('Model Similarity Spark SQL: Time vs Dataset Size (EMR Cluster)', fontsize=14)
