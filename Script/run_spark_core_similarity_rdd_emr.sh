...
SPARK_SCRIPT_PATH="/home/${SSH_USER}/pog/cluster/spark_core_similarity_cluster.py"

# Directory dove i log e i tempi verranno temporaneamente salvati sul nodo master EMR
LOCAL_LOG_DIR_ON_EMR="/home/${SSH_USER}/spark_logs_model_similarity_rdd_only/"
LOCAL_TIMES_FILE_ON_EMR="${LOCAL_LOG_DIR_ON_EMR}model_similarity_core_rdd_only_times.txt"
LOCAL_LOG_FILE_ON_EMR="${LOCAL_LOG_DIR_ON_EMR}model_similarity_core_rdd_only.txt"
...
LOCAL_GRAPH_FILE_PATH="${LOCAL_RESULTS_DIR}model_similarity_core_rdd_only_times.png"
LOCAL_TIMES_FILE_PATH="${LOCAL_RESULTS_DIR}model_similarity_core_rdd_only_times.txt"

# Assicurati che la directory locale per i risultati esista
mkdir -p "${LOCAL_RESULTS_DIR}"

echo "Starting Spark Model Similarity RDD-only job on EMR cluster..."
...
plt.title('Model Similarity Spark Core (RDD Only): Time vs Dataset Size (EMR Cluster)', fontsize=14)
