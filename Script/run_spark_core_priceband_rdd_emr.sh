...
SPARK_SCRIPT_PATH="/home/${SSH_USER}/pog/cluster/spark_core_priceband_cluster.py"

# Directory dove i log e i tempi verranno temporaneamente salvati sul nodo master EMR
LOCAL_LOG_DIR_ON_EMR="/home/${SSH_USER}/spark_logs_city_priceband_rdd/"
LOCAL_TIMES_FILE_ON_EMR="${LOCAL_LOG_DIR_ON_EMR}spark_core_city_priceband_rdd_only_times.txt"
LOCAL_LOG_FILE_ON_EMR="${LOCAL_LOG_DIR_ON_EMR}spark_core_city_priceband_rdd_only.txt"
...
LOCAL_GRAPH_FILE_PATH="${LOCAL_RESULTS_DIR}spark_core_city_priceband_rdd_only_times.png"
LOCAL_TIMES_FILE_PATH="${LOCAL_RESULTS_DIR}spark_core_city_priceband_rdd_only_times.txt"


mkdir -p "${LOCAL_RESULTS_DIR}"

echo "Starting Spark City-PriceBand RDD-only job on EMR cluster..."
echo "Results will be saved locally to: ${LOCAL_RESULTS_DIR}"

# Esegui lo script PySpark sul cluster EMR
# Senza `--conf` esplicite, Spark userà quelle di default di EMR.
...
# Ora, genera il grafico localmente
...
plt.title('Spark Core City-Year-PriceBand (RDD Only): Time vs Dataset Size (EMR Cluster)', fontsize=14)
